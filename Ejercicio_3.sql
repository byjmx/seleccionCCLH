select emp.NOMBRE_EMPRESA, format(lmed.FECHA_INICIO,'yyyy') "AÑO", count(emp.NOMBRE_EMPRESA) "Cantidad Licencias", format(sum(lmed.MONTO_REEMBOLSO),'###,###','de-de') "Suma monto reembolsado año"
